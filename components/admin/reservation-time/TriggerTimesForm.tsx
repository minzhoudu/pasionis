...
export const TriggerTimesForm = ({ times }: TriggerTimesFormProps) => {
    return (
        <div className="max-h-52 overflow-y-scroll rounded-md border p-5">
            <form className="flex flex-wrap justify-center gap-7">
                {times.map((reservationTime) => (
                    <button
                        key={reservationTime._id.toString()}
                        className={`w-16 rounded-md border px-2 py-1 text-center font-bold ${!reservationTime.available ? "border-red-500 bg-red-500 opacity-50" : ""}`}
